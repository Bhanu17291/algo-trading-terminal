...
def get_cash(db, profile: str) -> float:
    """
    Cash = INITIAL_CAPITAL
           + sum of all closed trade PnL
           - cost of any currently open trade
    This avoids reading equity curve (which includes MTM) as cash.
    """
    cash = float(INITIAL_CAPITAL)

    # Add PnL from all closed trades
    closed = (db.query(Trade)
                .filter(Trade.profile == profile, Trade.status == "CLOSED")
                .all())
    for t in closed:
        pnl = t.pnl_abs if t.pnl_abs is not None else 0.0
        cash += float(pnl)

    # Subtract cost of open trade (if any)
    open_trade = get_open_trade(db, profile)
    if open_trade:
        cash -= float(open_trade.position_size)
...
def days_held(entry_date_val) -> int:
    try:
        if isinstance(entry_date_val, str):
            entry = datetime.strptime(entry_date_val, "%Y-%m-%d").date()
        else:
            entry = entry_date_val
...
def run_profile_engine(db, profile: str, config: dict, signal: str,
    ...
    if open_trade is not None:
        ...
        sell_sig = signal == "HOLD" and confidence < 0.45

        if stop_hit or max_hit or sell_sig:
            qty     = int(open_trade.position_size / open_trade.entry_price)
            pnl_abs = (today_close - open_trade.entry_price) * qty
            ...
            db.flush()  # write before get_cash reads closed trades

            open_trade   = None
            action_taken = f"SELL ({exit_reason})"
            logger.info(f"[{profile}] SELL — {exit_reason}, PnL={pnl_abs:.0f}")

    # ── Recompute cash from source-of-truth (closed PnL - open cost) ─────────
    cash = get_cash(db, profile)

    # ── Check for entry ───────────────────────────────────────────────────────
    if open_trade is None and signal == "BUY" and confidence >= config["confidence_threshold"]:
        position_value = cash * config["position_fraction"]
        ...
        if qty > 0:
            ...
            db.flush()
            cash -= cost
            logger.info(f"[{profile}] BUY — price={today_close:.2f}, qty={qty}, cash_remaining={cash:.0f}")

    # ── Mark-to-market equity ─────────────────────────────────────────────────
    open_trade_now = get_open_trade(db, profile)
    mtm_value = 0.0
    if open_trade_now:
        qty_held  = int(open_trade_now.position_size / open_trade_now.entry_price)
    ...
    daily_return = ((total_equity - last_point.equity) / last_point.equity
                    if last_point and last_point.equity > 0 else 0.0)
...
def backfill_from_db():
    ...
    df = get_features_df()
    if df.empty:
        logger.error("[backfill] No data in DB — run data_pipeline_v3.py first.")
        return
    ...
    try:
        ...
        for i in range(1, len(df)):
            row         = df.iloc[[i]].copy()
            row_idx     = df.index[i]
            today_str   = str(row_idx.date()) if hasattr(row_idx, "date") else str(row_idx)
            today_close = float(df["close"].iloc[i])

            feat_row = row[FEATURES]
            if feat_row.isnull().any(axis=1).iloc[0]:
                continue

            feat_arr = feat_row.astype(float)

            p_xgb = xgb_model.predict_proba(feat_arr)[:, 1]
            p_lgb = lgbm_model.predict(feat_arr)
            p_cat = cat_model.predict_proba(feat_arr)[:, 1]
            buy_prob   = (weights["w_xgb"]  * float(p_xgb[0]) +
                          weights["w_lgbm"] * float(p_lgb[0]) +
                          weights["w_cat"]  * float(p_cat[0]))
